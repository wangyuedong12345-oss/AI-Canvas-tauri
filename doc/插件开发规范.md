...
> 适用宿主版本：ZeroFrame `0.9.0`；Plugin API v1 / v2 / v3<br>
> 最后更新：2026-08-30；插件入口：设置 → 插件 → 导入插件文件夹 / 插件市场

本文档说明如何为 ZeroFrame 编写用户插件。API v1 提供节点工具；API v2 增加自定义节点、宿主管理的模型调用和显式授权文件操作；API v3 增加由用户明确授权的可信 Python 运行时。
...
> **安全警告：** Python 插件不是沙箱。它以当前用户身份运行，可以绕过 Manifest 权限自行访问本机文件、网络和环境变量，也可以启动其他程序。Manifest 权限只约束 ZeroFrame 代办的模型、文件和画布能力。只安装并启用你信任且已审查源码的 Python 插件。
...
Python 插件在安装、更新和从停用状态重新启用时，都会先经过前端的来源、完整摘要、宿主权限和本机风险复核，再经过 Rust 原生 Warning；两个界面显示的完整摘要必须一致。确认表示用户信任这份本机代码，并不表示 ZeroFrame 已对其完成安全审计。
